...
- 群聊门禁的 `should_call_llm` 表示业务上的“允许调用”，AstrBot 的 `event.should_call_llm` 参数表示“禁止默认 LLM”；主流程通过反向映射连接两者，真实 @、唤醒前缀和回复机器人仍直接放行。
- 普通群聊的门禁按现有信息是否足以支持有内容的回复判断，不要求上下文一定提到 YeBot；发送前会丢弃未直接寻址场景中的纯澄清式反问。
...
- 2026-08-11：收紧普通群聊的可回答性判断，并在发送前过滤缺少上下文的澄清式反问。
- 2026-08-11：修正群聊门禁与 AstrBot 默认 LLM 开关的布尔值方向，恢复真实 @ 和唤醒前缀的主 Agent 调用。
